...
class ChangeParams(Callback):
    ...
    def _validate_params(self, params):
        ...
        for key, value in params.items():
            if key not in allowed_keys:
                raise ValueError(f"Unsupported top-level key '{key}'. Allowed keys are: {allowed_keys}")

            if key == "penalties":
                if not isinstance(value, list):
                    raise ValueError("'penalties' must be a list of dictionaries")
                for item in value:
                    if not isinstance(item, dict):
                        raise ValueError("Each item in 'penalties' must be a dictionary")
                    for subkey, subval in item.items():
                        self._validate_param_entry(f"{key}.{subkey}", subkey, subval)
            else:
    ...
    def _validate_param_entry(self, full_key, subkey, subval):
        if not isinstance(subkey, str):
            raise ValueError(f"Key '{full_key}' must be a string. Got {type(subkey)}")

        if isinstance(subval, str):
            if not (subval[0] in '+-*/' and self._is_number(subval[1:])):
                raise ValueError(f"String value '{subval}' in '{full_key}' must start with +, -, *, or / followed by a number")
        elif isinstance(subval, list):
            if not all(isinstance(x, (int, float)) for x in subval):
                raise ValueError(f"All elements in list for '{full_key}' must be int or float")
        else:
            raise ValueError(f"Invalid value type for '{full_key}': {type(subval)}. Can only be str or list.")
